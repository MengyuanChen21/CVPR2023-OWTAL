...
def train(itr, dataset, args, model, optimizer, device):
    ...
    outputs = model(features, seq_len=seq_len, is_training=True, itr=itr, opt=args, labels=labels)
    total_loss, loss_dict = model.criterion(outputs, labels, seq_len=seq_len, device=device, opt=args, itr=itr,
