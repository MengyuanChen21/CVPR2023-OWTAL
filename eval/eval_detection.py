...
class ANETdetection(object):
    ...
    def __init__(
        ...
        # 传入已知类个数
        self.n_known_class = args.n_known_class
    ...
    def _import_ground_truth(self, annotation_path):
        ...
        # classlist = np.load("./new_classlist.npy", allow_pickle=True)
        classlist = self.args.classlist
        ...
        # 计算Ground Truth的先验分布
        # prior = np.zeros((20, 1000))
        # for vid_idx in range(gtsegments.shape[0]):
        #     for seg_idx in range(len(gtsegments[vid_idx])):
        #         start_time = gtsegments[vid_idx][seg_idx][0]
        #         end_time = gtsegments[vid_idx][seg_idx][1]
        #         vid_duration = duration[vid_idx][0]
        #         start_time_percentage = round(start_time / vid_duration * 1000)
        #         end_time_percentage = round(end_time / vid_duration * 1000)
        #         seg_label = str2ind(gtlabels[vid_idx][seg_idx], self.classlist)
        #         prior[seg_label][start_time_percentage:end_time_percentage-1] += 1
        ...
        for i in range(len(gtsegments)):
            for j in range(len(gtsegments[i])):
                ...
                # 如果segment类别为Unknown类，将其类别置为15（第16类）
                this_label = str2ind(gtlabels[i][j], self.classlist)
                if this_label > self.n_known_class - 1:
                    this_label = self.n_known_class
                # label_lst.append(str2ind(gtlabels[i][j], self.classlist))
                label_lst.append(this_label)
    ...
    def wrapper_compute_average_precision(self):
        ...
        ap = np.zeros((len(self.tiou_thresholds), self.n_known_class + 1))
        ...
        results = Parallel(n_jobs=3)(
            delayed(compute_average_precision_detection)(
                ...
                prediction=self._get_predictions_with_label(
                    prediction_by_label, cidx
                ),
                tiou_thresholds=self.tiou_thresholds,
            )
            for cidx in range(self.n_known_class + 1)
        )
    ...
    def evaluate(self):
        ...
        # Known类别mAP
        known_mAP = self.ap[:, :-1].mean(axis=1)
        known_average_mAP = known_mAP.mean()

        # Unknown类别mAP
        unknown_mAP = self.ap[:, -1]
        unknown_average_mAP = unknown_mAP.mean()
